...
class PPOAgent:
    ...
    def __init__(self, number_of_inputs, number_of_actor_outputs, clip_param=0.2, max_grad_norm=0.5, ppo_update_iters=5,
        ...
        self.critic_optimizer = optim.Adam(self.critic_net.parameters(), critic_lr)
    ...
    def save(self, path):
        ...
        save(self.actor_optimizer.state_dict(), path + '_actor_opt.pkl')
        save(self.critic_optimizer.state_dict(), path + '_critic_opt.pkl')
    ...
    def load(self, path):
        ...
        actor_opt_state_dict = load(path + '_actor_opt.pkl')
        critic_opt_state_dict = load(path + '_critic_opt.pkl')
        self.actor_optimizer.load_state_dict(actor_opt_state_dict)
        self.critic_optimizer.load_state_dict(critic_opt_state_dict)
    ...
    def train_step(self, batch_size=None):
        ...
        state = tensor(np.array([t.state for t in self.buffer]), dtype=torch_float)
        action = tensor(np.array([t.action for t in self.buffer]), dtype=torch_long).view(-1, 1)
        reward = [t.reward for t in self.buffer]
        old_action_log_prob = tensor(np.array([t.a_log_prob for t in self.buffer]), dtype=torch_float).view(-1, 1)
